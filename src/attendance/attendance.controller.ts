import { Controller, Get, Post, Body, Patch, Param, Delete, HttpException, HttpStatus, UseGuards, Query, Request } from '@nestjs/common';
...
export class AttendanceController {
  ...
  @ApiResponse({ status: 200, description: 'Historial de asistencias obtenido correctamente', type: [Attendance] })
  @ApiResponse({ status: 401, description: 'No autorizado' })
  @ApiResponse({ status: 404, description: 'Usuario no encontrado' })
  async getUserAttendanceHistory(@Param('userId') userId: string, @Request() req) {
    try {
  ...
  @Post('check-in')
  @ApiOperation({ summary: 'Registrar asistencia mediante escaneo de QR (sin autenticación)' })
  @ApiResponse({ status: 201, description: 'Asistencia registrada correctamente', type: Attendance })
  @ApiResponse({ status: 400, description: 'Datos inválidos o usuario sin membresía activa' })
